...
public class TADialog extends Stage {
    ...
    private void initDialog() {
        ...
        GridPane gridPane = guiBuilder.buildGridPane(CSG_TA_DIALOG_GRID_PANE, null, CLASS_CSG_DIALOG_GRID_PANE, ENABLED);
        Label l1  = guiBuilder.buildLabel(CSG_TA_DIALOG_HEADER_LABEL, gridPane, 0, 0, 3, 1, CLASS_CSG_DIALOG_HEADER, ENABLED);
        Label l2 = guiBuilder.buildLabel(CSG_TA_DIALOG_NAME_LABEL, gridPane, 0, 1, 1, 1, CLASS_CSG_DIALOG_PROMPT, ENABLED);
        l1.setText(props.getProperty("CSG_TA_DIALOG_HEADER_LABEL_TEXT"));
        l2.setText(props.getProperty("CSG_TA_DIALOG_NAME_LABEL_TEXT"));
        TextField nameTextField = guiBuilder.buildTextField(CSG_TA_DIALOG_NAME_TEXT_FIELD, gridPane, 1, 1, 2, 1, CLASS_CSG_DIALOG_TEXT_FIELD, ENABLED);
        guiBuilder.buildLabel(CSG_TA_DIALOG_EMAIL_LABEL, gridPane, 0, 2, 1, 1, CLASS_CSG_DIALOG_PROMPT, ENABLED);
        TextField emailTextField = guiBuilder.buildTextField(CSG_TA_DIALOG_EMAIL_TEXT_FIELD, gridPane, 1, 2, 2, 1, CLASS_CSG_DIALOG_TEXT_FIELD, ENABLED);
        guiBuilder.buildLabel(CSG_TA_DIALOG_TYPE_LABEL, gridPane, 0, 3, 1, 1, CLASS_CSG_DIALOG_PROMPT, ENABLED);
        
        ToggleGroup tg = new ToggleGroup();
        guiBuilder.buildLabel(CSG_GRADUATE_DIALOG_LABEL, gridPane, 1, 3, 1, 1, CLASS_CSG_LABEL, ENABLED);
        RadioButton gradRadioButton = guiBuilder.buildRadioButton(CSG_TA_DIALOG_GRADUATE_RADIO_BUTTON, gridPane, 2, 3, 1, 1, CLASS_CSG_DIALOG_RADIO_BUTTON, ENABLED, tg, true);
        guiBuilder.buildLabel(CSG_UNDERGRADUATE_DIALOG_LABEL, gridPane, 3, 3, 1, 1, CLASS_CSG_LABEL, ENABLED);
        RadioButton undergradRadioButton = guiBuilder.buildRadioButton(CSG_TA_DIALOG_UNDERGRADUATE_RADIO_BUTTON, gridPane, 4, 3, 1, 1, CLASS_CSG_DIALOG_RADIO_BUTTON, ENABLED, tg, false);
        HBox okCancelBox = guiBuilder.buildHBox(CSG_TA_DIALOG_OK_BOX, gridPane, 0, 4, 3, 1, CLASS_CSG_DIALOG_BOX, ENABLED);
        okCancelBox.setAlignment(Pos.CENTER);
        Button okButton = guiBuilder.buildTextButton(CSG_TA_DIALOG_OK_BUTTON, okCancelBox, CLASS_CSG_DIALOG_BUTTON, !ENABLED);
        Button cancelButton = guiBuilder.buildTextButton(CSG_TA_DIALOG_CANCEL_BUTTON, okCancelBox, CLASS_CSG_DIALOG_BUTTON, ENABLED);
    ...
    private void processDataChange() {
        ...
        RadioButton radioButton = ((RadioButton)gui.getGUINode(CSG_TA_DIALOG_GRADUATE_RADIO_BUTTON));
    ...
    private void processCompleteWork() {
        ...
        RadioButton undergradRadioButton = ((RadioButton)app.getGUIModule().getGUINode(CSG_TA_DIALOG_UNDERGRADUATE_RADIO_BUTTON));
    ...
    public void showEditDialog(TeachingAssistantPrototype initTAToEdit) {
        ...
        if (initTAToEdit.getType().equals(TAType.Graduate.toString())) {
            ((RadioButton)app.getGUIModule().getGUINode(CSG_TA_DIALOG_GRADUATE_RADIO_BUTTON)).setSelected(true);
        }
        else {
            ((RadioButton)app.getGUIModule().getGUINode(CSG_TA_DIALOG_UNDERGRADUATE_RADIO_BUTTON)).setSelected(true);
        }
